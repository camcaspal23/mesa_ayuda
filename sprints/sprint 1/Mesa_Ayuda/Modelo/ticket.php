...
public function __CONSTRUCT(){
	...
	//proceso de encapsulamiento de variables de la base de datos.
	...
	public function Obtener($id){
		try {
			$consulta = $this->conn->prepare("SELECT t.idTicket, t.IdAprobacion, t.idNivelCaso, t.idUsuario, t.Descripcion,t.Solucion, t.FechaCreacion,t.FechaSolucion,t.usuarioSolucion FROM `dbo_mesa_ayuda`.`tb_ticket` t INNER JOIN  `dbo_mesa_ayuda`.`tb_estadoaprobacion` a ON(t.idAprobacion = a.idEstadoAprobacion) INNER JOIN  `dbo_mesa_ayuda`.`tb_nivelcaso` n ON(t.idNivelCaso = n.idNivelCaso) INNER JOIN  `dbo_mesa_ayuda`.`tb_usuario` u ON(t.idUsuario = u.idusuario) where t.idTicket = ?;");
	...
	public function Actualizar(ticket $t){
		//recibo valores $t del controlador
		try {
			$consulta = "UPDATE `dbo_mesa_ayuda`.`tb_ticket` SET `idEstadoCaso` = '3', `idAprobacion` = ?,`Descripcion` = ?, `Solucion` = ?, `idNivelCaso` = ?,`idUsuario` = ?,`FechaCreacion`=?,`FechaSolucion` = ?, `usuarioSolucion` = 'Analista.Mesa' WHERE `tb_ticket`.`idTicket` = ?;";
			$this->conn->prepare($consulta)->execute(array(
				//estos valores deben estar en orden dado que ? significa el valor en $t segun el get identificador.
