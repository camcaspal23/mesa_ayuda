...
class ticket
{
    ...
    {
        return $this->Id_CategoriaTicket;
    }

    public function setCategoria($Id_CategoriaTicket)
    {
        $this->Id_CategoriaTicket = $Id_CategoriaTicket;
    }
    ...
    {
        try {
            $consulta = "INSERT INTO `dbo_mesa_ayuda`.`tb_ticket` (`idTicket`, `idEstadoCaso`, `idAprobacion`, `Descripcion`, `Solucion`, 
                                                                    `idNivelCaso`, `idUsuario`, `FechaCreacion`, `FechaSolucion`, `usuarioSolucion`, `Id_CategoriaTicket`) 
                                                                    VALUES (NULL, '1', '3', ?, '', '4', ?, ?, '0000-00-00', '', ?);";
    ...
    {
        try {
            $consulta = $this->conn->prepare("SELECT t.idTicket, e.tipoEstado, u.usuario, a.TipoAprobacion, n.tipoCaso, t.usuarioSolucion, t.Descripcion, t.FechaCreacion 
                                                      FROM `dbo_mesa_ayuda`.`tb_ticket` t INNER JOIN  `dbo_mesa_ayuda`.`tb_estadocaso` e ON(t.idEstadoCaso = e.idEstadoCaso) 
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_usuario` u ON (t.idUsuario = u.idUsuario)
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_estadoaprobacion` a ON(t.idAprobacion = a.idEstadoAprobacion) 
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_nivelcaso` n ON(t.idNivelCaso = n.idNivelCaso)  where e.tipoestado = 'Abierto';");
            $consulta->execute();
    ...
    public function ListarUltimoAbierto()
    {
        try {
            $consulta = $this->conn->prepare("SELECT t.idTicket, e.tipoEstado, u.usuario, t.Descripcion, t.FechaCreacion
                                                      FROM `dbo_mesa_ayuda`.`tb_ticket` t INNER JOIN  `dbo_mesa_ayuda`.`tb_estadocaso` e ON(t.idEstadoCaso = e.idEstadoCaso)
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_usuario` u ON (t.idUsuario = u.idUsuario) where e.tipoestado = 'Abierto' ORDER BY t.idTicket DESC LIMIT 1 ;");
            $consulta->execute();
            return $consulta->fetchAll(PDO::FETCH_OBJ);
        } catch (Exception $e) {
            die($e->getMessage());
    ...
    {
        try {
            $consulta = $this->conn->prepare("SELECT t.idTicket, e.tipoEstado, u.usuario, a.TipoAprobacion, n.tipoCaso, t.usuarioSolucion, t.Solucion, t.FechaSolucion 
                                                      FROM `dbo_mesa_ayuda`.`tb_ticket` t INNER JOIN  `dbo_mesa_ayuda`.`tb_estadocaso` e ON(t.idEstadoCaso = e.idEstadoCaso) 
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_usuario` u ON (t.idUsuario = u.idUsuario)
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_estadoaprobacion` a ON(t.idAprobacion = a.idEstadoAprobacion) 
                                                      INNER JOIN  `dbo_mesa_ayuda`.`tb_nivelcaso` n ON(t.idNivelCaso = n.idNivelCaso)  where e.tipoestado = 'Solucionado';");
            $consulta->execute();
            return $consulta->fetchAll(PDO::FETCH_OBJ);
        } catch (Exception $e) {
            die($e->getMessage());
    ...
    public function Actualizar(ticket $t)
    {
        ...
        try {
            $consulta = "UPDATE `dbo_mesa_ayuda`.`tb_ticket` SET `idEstadoCaso` = '3', `idAprobacion` = ?,`Seguimiento` = ?,`Descripcion` = ?, 
                                                                  `Solucion` = ?, `idNivelCaso` = ?,`idUsuario` = ?,`FechaCreacion`=?,
                                                                  `FechaSolucion` = ?, `usuarioSolucion` = ? 
                                                                  WHERE `tb_ticket`.`idTicket` = ?;";
            $this->conn->prepare($consulta)->execute(array(
                $t->getIdAprobacion(),
                $t->getSeguimiento(),
                $t->getDescripcion(),
